...
extern "C" word __cdecl crc(char,word);
...
#ifdef PEX
   #define UPDC32(octet, crc) (crc_32_tab[int(((crc) ^ (octet)) & 0xff)] ^ (ul_shr((crc) , 8)))
#else
   #define UPDC32(octet, crc) (crc_32_tab[int(((crc) ^ (octet)) & 0xff)] ^ ((crc) >> 8))
#endif

dword
crc32(const void *ptr,unsigned len)
{
   dword crc = 0xFFFFFFFFLU;
   char *p = (char *)ptr;

   for(unsigned i=0;i<len;i++,p++) crc = UPDC32(*p,crc);
...
crc32(const char *s)
{
   ...
   for(;*s;s++) crc = UPDC32(*s, crc);

   return ~crc;
}

dword
upcrc32(const char *s)
{
   dword crc = 0xFFFFFFFFLU;

   for( ; *s ; s++) crc = UPDC32(toupper(*s), crc);
