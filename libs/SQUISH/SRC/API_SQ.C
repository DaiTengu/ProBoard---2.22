                                  ...
                                  dword clen, byte *ctxt)
{
  ...
  if (msg)
  {
    ...
    msg->ftsc_date[sizeof(msg->ftsc_date)-1]='\0';
...
static dword EXPENTRY SquishUidToMsgn(MSG *sq, UMSGID umsgid, word type)
{
  ...
  for (mn=0; mn < (word)sq->num_msg; mn++)
  {
    ...
    if (Sqd->idxbuf[mn].umsgid==umsgid ||
        ...
        (type==UID_PREV && Sqd->idxbuf[mn].umsgid <= umsgid &&
         ...
         (mn+1 >= (word)sq->num_msg || Sqd->idxbuf[mn+1].umsgid > umsgid)))
    {
      answer=(UMSGID)(mn+1);
      break;
    }
...
static sword near _SquishWriteIndex(MSG *sq)
{
  ...
  if (Sqd->idxbuf_delta==Sqd->idxbuf_write)
    ret=0;
           ...
           farwrite(Sqd->ifd,
                    ((char far *)Sqd->idxbuf) + (word)Sqd->idxbuf_delta,
                    (word)(Sqd->idxbuf_write - Sqd->idxbuf_delta))==-1)
  {
    ret=-1;
    msgapierr=MERR_BADF;
  }
