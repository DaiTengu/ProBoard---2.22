...
bool
simul_files_ra2(bool unlimited)
{
 RA2_FileArea ra_fa;
 _FileArea pb_fa;
 ...
 for(int rec=0; ;rec++)
   {
    ...
    ra_fa.NotFlags = swapbits(pb_fa.flagsNot);
    ra_fa.Security = pb_fa.level;

      ra_fa.Group = pb_fa.groups[0];
      ra_fa.AltGroup[0] = pb_fa.groups[1];
      ra_fa.AltGroup[1] = pb_fa.groups[2];
      ra_fa.AltGroup[2] = pb_fa.groups[3];
 ...
 for(int rec=0;;rec++)
   {
    ...
    if(pb_ma.msgBaseType == MSGBASE_HUDSON || pb_ma.msgBaseType == MSGBASE_JAM)
      {
       pb_ma.name[40] = '\0';
       ...
       ra_ma.ReadNotFlags = swapbits(*((long *)&pb_ma.readFlagsNot));
       ra_ma.WriteFlags = swapbits(*((long *)&pb_ma.writeFlags) );
       ra_ma.WriteNotFlags = swapbits(*((long *)&pb_ma.writeFlagsNot));
       ra_ma.SysopFlags = swapbits(*((long *)&pb_ma.sysopFlags) );
       ra_ma.SysopNotFlags = swapbits(*((long *)&pb_ma.sysopFlagsNot));
       ra_ma.ReadSecurity  = pb_ma.readLevel;
       ra_ma.WriteSecurity = pb_ma.writeLevel;
       ra_ma.SysopSecurity = pb_ma.sysopLevel;

       ra_ma.RecvKill  = pb_ma.rcvKillDays;
       ra_ma.DaysKill  = pb_ma.msgKillDays;
       ra_ma.CountKill = pb_ma.maxMsgs;

       ra_ma.MsgKinds   = pb_ma.msgType;
       ra_ma.Typ   = pb_ma.msgKind;

       ra_ma.Group = pb_ma.groups[0];
       ra_ma.AltGroup[0] = pb_ma.groups[1];
       ra_ma.AltGroup[1] = pb_ma.groups[2];
       ra_ma.AltGroup[2] = pb_ma.groups[3];
    ...
    ra_ma.Attribute = 2;
    if(ra_ma.Typ>=2) ra_ma.Attribute |= 1;

      if(pb_ma.msgBaseType == MSGBASE_JAM)
      {
      ra_ma.Attribute |= 0x80;
      pb_ma.path[60] = '\0';
      strcpy(ra_ma.JAMbase,pb_ma.path);
      c2pas(ra_ma.JAMbase);
      }
...
*/
 ...
 RA2_Config *racfg = new RA2_Config;
 CLEAR_OBJECT(*racfg);
